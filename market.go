...
func extractCandlesForSymbols(exchangeClient exchange.ExchangeClient, symbol string, date time.Time) (exchange.CandleBundle, error) {
	cb, err := exchangeClient.Candles1mByDate(symbol, date)
	if err != nil {
		return exchange.CandleBundle{}, fmt.Errorf("failed to get 1m candles for %s: %w", symbol, err)
	}
	return cb, nil
}
...
func TopMarketPositions(exchangeClient exchange.ExchangeClient) ([]Market, error) {
	...
	for i := range markets {
		...
		if len(candleBundle1h.Candles) > 0 {
			...
			lastPrice := candleBundle1h.Candles[1].Close
			markets[i].Change1h = roundTo(((lastPrice-openPrice)/openPrice)*100, 2)
		}
		...
		if len(candleBundle5m.Candles) > 0 {
			...
			lastPrice := candleBundle5m.Candles[1].Close
			markets[i].Change5m = roundTo(((lastPrice-openPrice)/openPrice)*100, 2)
		}*/
	}
