...
type Ticker24h struct {
	...
	CloseTimestamp int    `json:"closeTimestamp"`
}
...
type StakingBalanceResponse struct {
	Action   string           `json:"action"`
	Response []StakingBalance `json:"response"`
}
...
type Websocket struct {
	...
	stakingBalanceChannel    chan []StakingBalance
...
func (bitvavo Bitvavo) PlaceOrder(market string, side string, orderType string, body map[string]string) (Order, error) {
	...
	jsonResponse := bitvavo.sendPrivate("/order", "", requestBody, "POST")
	var t Order
	err := json.Unmarshal(jsonResponse, &t)
	if err != nil {
...
func (bitvavo Bitvavo) UpdateOrder(market string, orderId string, body map[string]string) (Order, error) {
	...
	jsonResponse := bitvavo.sendPrivate("/order", "", requestBody, "PUT")
	var t Order
	err := json.Unmarshal(jsonResponse, &t)
	if err != nil {
		return Order{}, MyError{Err: err}
...
func (bitvavo Bitvavo) CancelOrder(market string, orderId string, options map[string]string) (CancelOrder, error) {
	if options == nil {
...
// options: symbol
func (bitvavo Bitvavo) StakingBalance(options map[string]string) ([]StakingBalance, error) {
	postfix := bitvavo.createPostfix(options)
	jsonResponse := bitvavo.sendPrivate("/stakingBalance", postfix, map[string]any{}, "GET")
	t := make([]StakingBalance, 0)
	err := json.Unmarshal(jsonResponse, &t)
	if err != nil {
		return []StakingBalance{}, handleAPIError(jsonResponse)
	}
	return t, nil
}
...
func (bitvavo Bitvavo) WithdrawAssets(symbol string, amount string, address string, body map[string]string) (WithdrawAssets, error) {
	...
	jsonResponse := bitvavo.sendPrivate("/withdrawal", "", requestBody, "POST")
...
func (bitvavo Bitvavo) handleMessage(ws *Websocket) {
	for {
		...
		} else if x["action"] == "privateGetStakingBalance" {
			var t StakingBalanceResponse
			err = json.Unmarshal(message, &t)
			if handleError(err) {
				return
			}
			ws.stakingBalanceChannel <- t.Response
...
func (ws *Websocket) PlaceOrder(market string, side string, orderType string, body map[string]string) chan Order {
	...
	requestBody["side"] = side
	requestBody["orderType"] = orderType
	requestBody["action"] = "privateCreateOrder"
	myMessage, _ := json.Marshal(requestBody)
	go ws.sendPrivate(myMessage)
	return ws.placeOrderChannel
}

func normalizeRequestBody(body map[string]string) map[string]any {
	result := make(map[string]any, len(body))
	for k, v := range body {
		switch k {
		case "requestId", "operatorId":
			result[k] = mustParseIntParam(k, v)
		case "postOnly", "disableMarketProtection", "responseRequired":
			result[k] = mustParseBoolParam(k, v)
		default:
			result[k] = v
		}
	}
...
func mustParseIntParam(key string, value string) int {
	result, err := strconv.Atoi(value)
	if err != nil {
		panic("Failed to parse int parameter " + key + ": " + err.Error())
	}
	return result
}

func mustParseBoolParam(key string, value string) bool {
	result, err := strconv.ParseBool(value)
	if err != nil {
		panic("Failed to parse bool parameter " + key + ": " + err.Error())
	}
	return result
}
...
func (ws *Websocket) UpdateOrder(market string, orderId string, body map[string]string) chan Order {
	...
	requestBody["market"] = market
	requestBody["orderId"] = orderId
	requestBody["action"] = "privateUpdateOrder"
	myMessage, _ := json.Marshal(requestBody)
	go ws.sendPrivate(myMessage)
	return ws.updateOrderChannel
}

func (ws *Websocket) CancelOrder(market string, orderId string, options map[string]string) chan CancelOrder {
	ws.cancelOrderChannel = make(chan CancelOrder, 100)
	requestBody := normalizeRequestBody(options)
	requestBody["action"] = "privateCancelOrder"
	requestBody["market"] = market
	requestBody["orderId"] = orderId
	myMessage, _ := json.Marshal(requestBody)
	go ws.sendPrivate(myMessage)
...
func (ws *Websocket) CancelOrders(options map[string]string) chan []CancelOrder {
	...
	requestBody := normalizeRequestBody(options)
	requestBody["action"] = "privateCancelOrders"
	myMessage, _ := json.Marshal(requestBody)
	go ws.sendPrivate(myMessage)
...
func (ws *Websocket) StakingBalance(options map[string]string) chan []StakingBalance {
	ws.stakingBalanceChannel = make(chan []StakingBalance, 100)
	options["action"] = "privateGetStakingBalance"
	myMessage, _ := json.Marshal(options)
	go ws.sendPrivate(myMessage)
	return ws.stakingBalanceChannel
}
